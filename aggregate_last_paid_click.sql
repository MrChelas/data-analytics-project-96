...
vk1 as (
	...
	from vk_ads
	group by date_vk, utm_source, utm_medium, utm_campaign
),
...
ya1 as (
	...
	from ya_ads
	group by date_ya, utm_source, utm_medium, utm_campaign
),
...
purchases as (
	...
	from (
			select *, row_number() over(partition by visitor_id order by visit_date desc) as rn
	  		from sessions) s
	left join leads l
		on s.visitor_id = l.visitor_id
	where s.medium in ('cpc', 'cpm', 'cpa', 'youtube', 'cpp', 'tg', 'social') and rn = 1
	group by date_trunc('day', s.visit_date), source, medium, campaign
)
